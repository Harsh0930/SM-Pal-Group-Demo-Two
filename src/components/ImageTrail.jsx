...
/* ─── Variant 2: scale in from cursor, shrink on fade ─── */
...
/* ─── Variant 6: speed-scaled brightness & blur ─── */
class ImageTrailVariant6 {
  ...
  mapSpeedToSize(speed, min, max) {
    return min + (max - min) * Math.min(speed / 200, 1);
  }
  ...
  showNextImage() {
    ...
    const scaleFactor = this.mapSpeedToSize(speed, 0.3, 1.8);
    ...
    gsap
      ...
      .fromTo(
        img.DOM.inner,
        { scale: 2 },
        { duration: 0.75, ease: 'power3', scale: 1 },
        0,
      )
      .to(
        img.DOM.el,
